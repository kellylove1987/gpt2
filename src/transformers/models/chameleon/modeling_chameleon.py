...
class ChameleonModel(ChameleonPreTrainedModel):
    ...
    def get_image_tokens(self, pixel_values: torch.FloatTensor):
        ...
        Returns:
            `torch.Tensor` of shape `(batch_size, vqmodel.quant_state_flattened_dim)`:
    ...
    def decode_image_tokens(self, bpe_tokens: torch.Tensor):
        ...
        Args:
            bpe_tokens (`torch.tensor` of shape `(batch, vqmodel.quant_state_flattened_dim)`):
                The BPE tokens generated by the model.

        Returns:
            `torch.Tensor` of shape `(batch, num_channels, 512, 512)`:
        """
        if bpe_tokens.shape[1] != self.vqmodel.quant_state_flattened_dim:
            raise ValueError(f"All batches must have {self.vqmodel.quant_state_flattened_dim} tokens.")
