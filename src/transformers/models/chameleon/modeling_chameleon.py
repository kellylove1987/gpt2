...
class ChameleonVQVAE(PreTrainedModel):
    ...
    def encode(self, pixel_values: torch.LongTensor):
        ...
        Returns:
            quant (`torch.FloatTensor` of shape `(batch_size, embed_dim, 32, 32)`):
                Embeddings of quantized tokens.
            emb_loss (`torch.FloatTensor`):
                Embedding loss.
            indices (`torch.LongTensor` of shape `(batch_size, 1024)`):
                Token IDs
        """
    ...
    def decode(self, image_tokens: torch.Tensor):
        ...
        Args:
            image_tokens (`torch.LongTensor` of shape `(batch_size, 1024)`):
                Batch of token IDs.
...
class ChameleonImageVocabularyMapping:
    ...
    def convert_bpe2img(self, bpe_batch: torch.Tensor) -> torch.Tensor:
        ...
        return img_tok[torch.searchsorted(bpe_tok, bpe_batch.to("cpu"))].to(device)
...
class ChameleonModel(ChameleonPreTrainedModel):
    ...
    def get_image_tokens(self, pixel_values: torch.FloatTensor):
        ...
        Returns:
            `torch.Tensor` of shape `(batch_size, 1024)`:
                The BPE tokens generated by the model.
        """
        _, _, image_toks = self.vqmodel.encode(pixel_values)
        return self.vocabulary_mapping.convert_img2bpe(image_toks)

    def decode_image_tokens(self, bpe_tokens: torch.Tensor):
        """
        Decodes BPE image tokens into PIL Image with VQGAN module.

        Args:
            bpe_tokens (`torch.tensor` of shape `(batch, 1024)`):
                The BPE tokens generated by the model.
        """
        if bpe_tokens.shape[1] != 1024:
            raise ValueError("All batches must have 1024 tokens.")
