...
class LlamaAttention(nn.Module):
    ...
    def __init__(self, config: LlamaConfig, layer_idx: Optional[int] = None):
        ...
        # Otherwise it is done kv_channels * num_key_value_heads -> hidden_size
        if config.kv_channels is None:
            self.o_proj = nn.Linear(self.hidden_size, self.hidden_size, bias=config.attention_bias)
        else:
            self.o_proj = nn.Linear(self.hidden_size, config.kv_channels * self.num_key_value_heads, bias=config.attention_bias)
